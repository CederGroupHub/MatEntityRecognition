...
def evaluate(correctChunk, foundGuessed, foundCorrect, correctTags, tokenCounter, latex=False):
    ...
    if not latex:
        ...
        info += "processed %i tokens with %i phrases; " % (tokenCounter, foundCorrectSum)
        info += "found: %i phrases; correct: %i.\n" % (foundGuessedSum, correctChunkSum)
        # print("processed %i tokens with %i phrases; " % (tokenCounter, foundCorrectSum), end='')
        # print("found: %i phrases; correct: %i.\n" % (foundGuessedSum, correctChunkSum), end='')
        if tokenCounter:
            info += "accuracy: %6.2f%%; " % (100*correctTags/tokenCounter)
            info += "precision: %6.2f%%; recall: %6.2f%%; FB1: %6.2f" % \
    ...
    # http://cnts.uia.ac.be/conll2003/ner/example.tex
        ...
        info += "        & Precision &  Recall  & F\$_{\\beta=1} \\\\\\hline" 
        ...
        precision, recall, FB1 = calcMetrics(correctChunkSum, foundGuessedSum, foundCorrectSum)
        info += "Overall &  %6.2f\\%% & %6.2f\\%% & %6.2f \\\\\\hline" % \
              (precision,recall,FB1)
        # print("Overall &  %6.2f\\%% & %6.2f\\%% & %6.2f \\\\\\hline" %
              # (precision,recall,FB1))
    return info 

def evaluate_file(file_in, file_out):
    # process input and count chunks
    with open(file_in, 'r') as fr:
        correctChunk, foundGuessed, foundCorrect, correctTags, tokenCounter = countChunks(fr)
    # correctChunk, foundGuessed, foundCorrect, correctTags, tokenCounter = countChunks(sys.stdin, args)

    # compute metrics and print
    info = evaluate(correctChunk, foundGuessed, foundCorrect, correctTags, tokenCounter)    
    with open(file_out, 'w') as fw:
        fw.write(info)

def evaluate_lines(input_lines):
    # process input and count chunks
    correctChunk, foundGuessed, foundCorrect, correctTags, tokenCounter = countChunks(input_lines)
...
if __name__ == "__main__":
    # args = parse_args()
    # # process input and count chunks
    # fr = open('eval.1373703.output', 'r')
    # correctChunk, foundGuessed, foundCorrect, correctTags, tokenCounter = countChunks(fr, args)
    # # correctChunk, foundGuessed, foundCorrect, correctTags, tokenCounter = countChunks(sys.stdin, args)
    ...
    evaluate_file('eval.1000241.output', 'eval.1000241.scores2')
